...
\begin{figure}[htbp]
    \begin{center}
        ...
        \caption{A one-parameter subgroup showing $a$, the tangent at $e$, and $\dot{p}(t)$, the vector into which $v^{\alpha}_{\beta}\paren{p(t)}$ transforms $a$.}
        \label{tab:fig 3-1}
    \end{center}
\end{figure}
It is evident from \eqref{eq:3-10} that $a(0) = e$ and that $[a(t)]^{-1} = a(-t)$. The one-parameter subgroup can be shown to be isomorphic to either R, the group of real numbers under addition, or to R/Z, the factor group discussed in Section 3 of this chapter, depending on whether or not there is a number t1 such that a(t1) =e.
